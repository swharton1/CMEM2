...
def get_rmp0(a, beta_c, c, dn, ds, theta_n, theta_s, r0_lin, p0, p2, p3):
        '''This will get the subsolar magnetopause distance for the current parameters. 
        ...
        Q0 = c*(np.exp(p3*dn*np.cos(theta_n)) + np.exp(p3*ds*np.cos(theta_s))) 
...
def get_model_func(current_model):
        ...
        if current_model == "jorg":
        ...
        elif current_model == "cmem2":
            def cmem2_func(r, theta, phi, a, beta_c, c, dn, ds, theta_n, theta_s, r0_lin, p0, dr, A1, A2, B, dbeta, beta, p1, p2, p3, ay_bs, az_bs):
                '''
                This is the CMEM model, which will use the lin model to work out 
                the magnetopause location instead of the shue model. 
                ...
                #Magnetosheath.
                msheath = A1*(np.exp(-B*(theta/2.)**4))*((r/10)**(-dbeta-(beta*(np.cos(theta))**2)))/(1 + np.exp(-k*(r-rmp)))*(1+np.exp(k*(r-rbs)))
                
                #Solar wind. 
                swind = A2*((r/10)**(-3))/(1 + np.exp(-k*(r-rbs)))
            ...
            raise ValueError("{} not a valid model. 'jorg', 'cmem' or 'cmem2' only atm.".format(current_model))
